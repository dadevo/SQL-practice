...
--Q8: List all classes from highest scoring to lowest scoring, per track.
...
SELECT track_number, track_id, ROUND(AVG(average_score),2) as average_score
FROM students
INNER JOIN [average_score]
ON students.id = [average_score].student_id
GROUP BY track_number, track_id
